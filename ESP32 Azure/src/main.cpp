...
//// Differnce between this code and the original example for the Espressif ESP32 Azure IoT Kit /////
/*
Dependencies Section:

Both snippets include C99 libraries, mbedtls libraries for SHA256 encryption, MQTT client and WiFi libraries, and Azure IoT SDK for C libraries.
The first snippet includes <cstdlib> for general utilities, whereas the second snippet includes <cstdarg> for handling variable arguments in functions.
The first snippet includes the <time.h> library for time-related functions, whereas the second snippet does not.
Sample-specific Settings:

Both snippets define sample-specific settings such as serial logger baud rate and MQTT message retention policy.
The second snippet additionally defines settings related to NTP servers, time zones, and UNIX time.
Function Declarations:

Both snippets declare functions for syncing device clock with NTP server, connecting to WiFi, and handling MQTT events.
The first snippet declares a logging function used by Azure IoT client, whereas the second snippet does not.
Sample Variables:

Both snippets declare variables for Azure IoT configuration, MQTT client handle, and buffers for data.
The second snippet additionally declares variables for properties request ID and a boolean flag.
MQTT Interface Functions:

Both snippets provide implementations for MQTT interface functions used by Azure IoT.
The first snippet includes implementation for subscribing and publishing MQTT messages, whereas the second snippet does not include these functions.
Other Interface Functions Required by Azure IoT:

Both snippets include implementations for HMAC SHA256 encryption, base64 encoding/decoding, handling properties update, and handling command requests.
The first snippet additionally defines UNIX time constants and provides an implementation for MQTT client initialization and deinitialization functions.
Arduino Setup and Loop Functions:

Both snippets include the setup() function where serial communication is initialized, logging function is set, WiFi connection is established, NTP sync is performed, and Azure IoT is initialized.
The second snippet additionally sets the telemetry frequency for Azure IoT
*/
///////////////////////////////////////////////////////////////////////////////////////////////
/* --- Dependencies --- */
// C99 libraries
